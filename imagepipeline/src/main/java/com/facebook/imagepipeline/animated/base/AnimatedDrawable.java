...
public class AnimatedDrawable extends Drawable implements AnimatableDrawable, DrawableWithCaches {
    ...
    public AnimatedDrawable(ScheduledExecutorService scheduledExecutorServiceForUiThread, AnimatedDrawableCachingBackend animatedDrawableBackend,
                            AnimatedDrawableDiagnostics animatedDrawableDiagnostics, MonotonicClock monotonicClock) {
    ...
    private void computeAndScheduleNextFrame(boolean scheduleNextFrame) {
        ...
        if (changed) {
            ...
            int durationMs = mAnimatedDrawableBackend.getTimestampMsForFrame(mScheduledFrameNumber) + mAnimatedDrawableBackend
                    .getDurationMsForFrame(mScheduledFrameNumber) - timestampMs;
    ...
    public void draw(Canvas canvas) {
        ...
        try {
            ...
            if (mIsRunning && !mHaveWatchdogScheduled) {
                mScheduledExecutorServiceForUiThread.schedule(mWatchdogTask, WATCH_DOG_TIMER_POLL_INTERVAL_MS, TimeUnit.MILLISECONDS);
                mHaveWatchdogScheduled = true;
            }
            if (mApplyTransformation) {
                mDstRect.set(getBounds());
                if (!mDstRect.isEmpty()) {
                    AnimatedDrawableCachingBackend newBackend = mAnimatedDrawableBackend.forNewBounds(mDstRect);
            ...
            if (mPendingRenderedFrameNumber != NO_FRAME) {
                ...
                boolean rendered = renderFrame(canvas, mPendingRenderedFrameNumber, mPendingRenderedFrameMonotonicNumber);
            ...
            if (mPendingRenderedFrameNumber == NO_FRAME) {
                ...
                boolean rendered = renderFrame(canvas, mScheduledFrameNumber, mScheduledFrameMonotonicNumber);
                didDrawFrame |= rendered;
                if (rendered) {
            ...
            if (!didDrawFrame) {
                ...
                CloseableReference<Bitmap> previewBitmapReference = mAnimatedDrawableBackend.getPreviewBitmap();
    ...
    private boolean renderFrame(Canvas canvas, int frameNumber, int frameMonotonicNumber) {
        CloseableReference<Bitmap> bitmapReference = mAnimatedDrawableBackend.getBitmapForFrame(frameNumber);
    ...
    private void doWatchdogCheck() {
        ...
        boolean hasNotDrawnWithinTimeout = mWaitingForDraw && now - mLastInvalidateTimeMs > WATCH_DOG_TIMER_MIN_TIMEOUT_MS;
        // Also timeout onNextFrame is more than 2 seconds late.
        boolean hasNotAdvancedFrameWithinTimeout = mNextFrameTaskMs != -1 && now - mNextFrameTaskMs > WATCH_DOG_TIMER_MIN_TIMEOUT_MS;
