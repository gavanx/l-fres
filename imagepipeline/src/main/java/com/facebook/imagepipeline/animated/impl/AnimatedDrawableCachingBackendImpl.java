...
public class AnimatedDrawableCachingBackendImpl extends DelegatingAnimatedDrawableBackend implements AnimatedDrawableCachingBackend {
    ...
    @GuardedBy("this") private final List<Bitmap> mFreeBitmaps;
    @GuardedBy("this") private final SparseArrayCompat<Task<Object>> mDecodesInFlight;
    @GuardedBy("this") private final SparseArrayCompat<CloseableReference<Bitmap>> mCachedBitmaps;
    @GuardedBy("this") private final WhatToKeepCachedArray mBitmapsToKeepCached;
    @GuardedBy("ui-thread") private int mCurrentFrameIndex;

    public AnimatedDrawableCachingBackendImpl(SerialExecutorService executorService, ActivityManager activityManager, AnimatedDrawableUtil
            animatedDrawableUtil, MonotonicClock monotonicClock, AnimatedDrawableBackend animatedDrawableBackend, AnimatedDrawableOptions options) {
        ...
        mMaximumKiloBytes = options.maximumBytes >= 0 ? options.maximumBytes / ByteConstants.KB : getDefaultMaxBytes(activityManager) /
                ByteConstants.KB;
        ...
        mApproxKiloBytesToHoldAllFrames = mAnimatedDrawableBackend.getRenderedWidth() * mAnimatedDrawableBackend.getRenderedHeight() /
                ByteConstants.KB *
    ...
    private Bitmap createNewBitmap() {
        ...
        return Bitmap.createBitmap(mAnimatedDrawableBackend.getRenderedWidth(), mAnimatedDrawableBackend.getRenderedHeight(), Bitmap.Config
                .ARGB_8888);
    }
    ...
    public AnimatedDrawableCachingBackend forNewBounds(Rect bounds) {
        ...
        return new AnimatedDrawableCachingBackendImpl(mExecutorService, mActivityManager, mAnimatedDrawableUtil, mMonotonicClock, newBackend,
    ...
    private void runPrefetch(int frameNumber) {
        ...
        CloseableReference<Bitmap> preDecodedFrame = mAnimatedDrawableBackend.getPreDecodedFrame(frameNumber);
        try {
    ...
    private synchronized CloseableReference<Bitmap> getCachedOrPredecodedFrame(int frameNumber) {
        CloseableReference<Bitmap> ret = CloseableReference.cloneOrNull(mCachedBitmaps.get(frameNumber));
        if (ret == null) {
    ...
    private synchronized boolean hasCachedOrPredecodedFrame(int frameNumber) {
        return mCachedBitmaps.get(frameNumber) != null || mAnimatedDrawableBackend.hasPreDecodedFrame(frameNumber);
    }
