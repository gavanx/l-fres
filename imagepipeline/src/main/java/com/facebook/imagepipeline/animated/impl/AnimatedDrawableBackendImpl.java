...
public class AnimatedDrawableBackendImpl implements AnimatedDrawableBackend {
    ...
    @GuardedBy("this") private Bitmap mTempBitmap;
    ...
    private void renderImageSupportsScaling(Canvas canvas, AnimatedImageFrame frame) {
        ...
        synchronized (this) {
            if (mTempBitmap == null) {
                mTempBitmap = Bitmap.createBitmap(mRenderedBounds.width(), mRenderedBounds.height(), Bitmap.Config.ARGB_8888);
            }
    ...
    public void renderImageDoesNotSupportScaling(Canvas canvas, AnimatedImageFrame frame) {
        ...
        synchronized (this) {
            if (mTempBitmap == null) {
                mTempBitmap = Bitmap.createBitmap(mAnimatedImage.getWidth(), mAnimatedImage.getHeight(), Bitmap.Config.ARGB_8888);
            }
            mTempBitmap.eraseColor(Color.TRANSPARENT);
            frame.renderFrame(frameWidth, frameHeight, mTempBitmap);
