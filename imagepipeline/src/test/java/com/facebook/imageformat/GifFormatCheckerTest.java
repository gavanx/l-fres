...
public class GifFormatCheckerTest {
    ...
    public void testCircularBufferMatchesBytePattern() {
        byte[] testBytes = new byte[]{(byte) 0x00, (byte) 0x01, (byte) 0x02, (byte) 0x03, (byte) 0x04};
        ...
        for (int i = 2; i < testBytes.length; i++) {
            assertFalse(GifFormatChecker.circularBufferMatchesBytePattern(testBytes, i, testInnerBytes));
        }
        byte[] testCircularBytes = new byte[]{(byte) 0x04, (byte) 0x00};
        assertTrue(GifFormatChecker.circularBufferMatchesBytePattern(testBytes, 4, testCircularBytes));
        for (int i = 0; i < 4; i++) {
            assertFalse(GifFormatChecker.circularBufferMatchesBytePattern(testBytes, i, testCircularBytes));
        }
    }

    private void singleAnimatedGifTest(final List<String> resourceNames, final boolean expectedAnimated) throws Exception {
        for (String name : resourceNames) {
            final InputStream resourceStream = getResourceStream(name);
            try {
                assertSame("failed with resource: " + name, expectedAnimated, GifFormatChecker.isAnimated(resourceStream));
            } finally {
