...
public class WebpBitmapFactoryImpl implements WebpBitmapFactory {
    public static final boolean IN_BITMAP_SUPPORTED = Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB;
    ...
    public static Bitmap hookDecodeByteArray(byte[] array, int offset, int length, BitmapFactory.Options opts) {
        ...
        if (isWebpHeader(array, offset, length) && !isWebpPlatformSupported(array, offset, length)) {
            bitmap = nativeDecodeByteArray(array, offset, length, opts, getScaleFromOptions(opts), getInBitmapFromOptions(opts),
    ...
    public static Bitmap hookDecodeStream(InputStream inputStream, Rect outPadding, BitmapFactory.Options opts) {
        ...
        if (isWebpHeader(header, 0, HEADER_SIZE) && !isWebpPlatformSupported(header, 0, HEADER_SIZE)) {
            bitmap = nativeDecodeStream(inputStream, opts, getScaleFromOptions(opts), getInBitmapFromOptions(opts), getInTempStorageFromOptions
                    (opts));
            setWebpBitmapOptions(bitmap, opts);
    ...
    public static Bitmap hookDecodeFileDescriptor(FileDescriptor fd, Rect outPadding, BitmapFactory.Options opts) {
        ...
        if (originalSeekPosition != -1) {
            ...
            try {
                byte[] header = getWebpHeader(inputStream, opts);
                if (isWebpHeader(header, 0, HEADER_SIZE) && !isWebpPlatformSupported(header, 0, HEADER_SIZE)) {
                    bitmap = nativeDecodeStream(inputStream, opts, getScaleFromOptions(opts), getInBitmapFromOptions(opts),
                            getInTempStorageFromOptions(opts));
                    setPaddingDefaultValues(outPadding);
                    setWebpBitmapOptions(bitmap, opts);
                } else {
    ...
    @DoNotStrip
    private static native Bitmap nativeDecodeStream(InputStream is, BitmapFactory.Options options, float scale, Bitmap inBitmap, byte[]
            inTempStorage);

    @DoNotStrip
    private static native Bitmap nativeDecodeByteArray(byte[] data, int offset, int length, BitmapFactory.Options opts, float scale, Bitmap
            inBitmap, byte[] inTempStorage);
