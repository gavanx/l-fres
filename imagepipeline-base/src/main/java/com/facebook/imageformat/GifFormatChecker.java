    ...
    public static boolean isAnimated(InputStream source) {
        ...
        try {
            ...
            while (source.read(buffer, offset, 1) > 0) {
                ...
                if (circularBufferMatchesBytePattern(buffer, offset + 1, FRAME_HEADER_START) && (circularBufferMatchesBytePattern(buffer, offset +
                        9, FRAME_HEADER_END_1) || circularBufferMatchesBytePattern(buffer, offset + 9, FRAME_HEADER_END_2))) {
                    frameHeaders++;
                    if (frameHeaders > 1) {
                        return true;
                    }
