...
public class DefaultZoomableController implements ZoomableController, TransformGestureDetector.Listener {
    ...
    private void setTransformAnimated(final Matrix newTransform, long durationMs, @Nullable final Runnable onAnimationComplete) {
        ...
        mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                for (int i = 0; i < mAnimationCurrValues.length; i++) {
                    mAnimationCurrValues[i] = (1 - fraction) * mAnimationStartValues[i] + fraction * mAnimationDestValues[i];
                }
     ...
     */
    public void zoomToImagePoint(float scale, PointF imagePoint, PointF viewPoint, boolean limitTransX, boolean limitTransY, long durationMs,
  ...
  /* TransformGestureDetector.Listener methods  */
    ...
    private boolean limitTranslation(Matrix newTransform, boolean limitX, boolean limitY) {
        ...
        float offsetLeft = limitX ? getOffset(bounds.left, bounds.width(), mViewBounds.width()) : bounds.left;
        float offsetTop = limitY ? getOffset(bounds.top, bounds.height(), mViewBounds.height()) : bounds.top;
